...
export default function Dashboard() {
  ...
  const sortedAndFilteredCampaigns = useMemo(() => {
    ...
    if (selectedStatus !== 'Todos os Status') {
      ...
      filtered = filtered.filter((campaign: MetaCampaign) => 
        validStatuses.includes(campaign.status)
      );
    }
    
    // Aplicar ordenação: Conta > Status (Ativas primeiro) > Valor Gasto (decrescente)
    return filtered.sort((a: MetaCampaign, b: MetaCampaign) => {
      // 1. Ordenar por Conta (teamName) alfabeticamente
      const accountCompare = a.teamName.localeCompare(b.teamName);
      if (accountCompare !== 0) return accountCompare;
      
      // 2. Dentro da mesma conta, ordenar por status (Ativas primeiro)
      const getStatusPriority = (status: string) => {
        const statusLower = status.toLowerCase();
        if (statusLower === 'active' || statusLower === 'ativa') return 0;
        if (statusLower === 'paused' || statusLower === 'pausada') return 1;
        return 2; // Outros status por último
      };
      
      const statusCompare = getStatusPriority(a.status) - getStatusPriority(b.status);
      if (statusCompare !== 0) return statusCompare;
      
      // 3. Por fim, ordenar por valor gasto (decrescente)
  ...
  // Obter contas únicas para o filtro
  ...
  const calculateCPL = (spend: string, leads: number) => {
    const spendNum = parseFloat(spend);
    if (leads === 0 || spendNum === 0) return '0.00';
    return (spendNum / leads).toFixed(2);
  };
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <KpiCard
                  ...
                  value={`R$ ${metaSummary?.totalSpend ? Number(metaSummary.totalSpend).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '0,00'}`}
                  icon={<DollarSign className="h-6 w-6 text-[#c89968]" />}
                />
                <KpiCard
                  title="Impressões"
                  value={metaSummary?.totalImpressions ? metaSummary.totalImpressions.toLocaleString('pt-BR') : '0'}
                  icon={<Eye className="h-6 w-6 text-[#c89968]" />}
                />
                <KpiCard
                  title="Cliques"
                  value={metaSummary?.totalClicks ? metaSummary.totalClicks.toLocaleString('pt-BR') : '0'}
                  icon={<MousePointer className="h-6 w-6 text-[#c89968]" />}
                />
                <KpiCard
                  title="Leads Gerados"
                  value={metaSummary?.totalLeads ? metaSummary.totalLeads.toLocaleString('pt-BR') : '0'}
              ...
              <div className="bg-white/90 backdrop-blur rounded-2xl border border-[#684e3a]/20 overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs sm:text-sm text-[#3d2e28]">
                    <thead className="bg-[#FAF9F6] text-[10px] sm:text-[11px] uppercase font-semibold tracking-[0.18em] text-[#684e3a]">
                      <tr>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Nome da Campanha</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Status</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Início</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Fim</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Conta / Operação</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Valor Gasto</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Impressões</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Cliques</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">CTR (%)</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">Leads</th>
                        <th className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">CPL</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 font-medium text-[#3d2e28] max-w-[200px] truncate">
                              <span title={campaign.campaign_name} className="truncate cursor-help">
                            ...
                            <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs text-[#3d2e28] whitespace-nowrap">
                            ...
                            <td className="px-3 sm:px-6 py-3 sm:py-4 text-xs text-[#3d2e28]">
  ...
  return (
    <div className="bg-white/95 p-6 rounded-2xl border border-[#684e3a]/20 flex items-start justify-between transition-all duration-200 hover:-translate-y-0.5 hover:border-[#c89968] hover:shadow-md">
      <div>
        ...
        <h4 className="text-xl sm:text-2xl font-semibold text-[#3d2e28]">{value}</h4>
      </div>
